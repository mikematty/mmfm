  ...
  while(!(length_done && space_done && passes_done)) {
    ...
    if (buttons) {
      if (buttons & BUTTON_UP) {
        if(!length_done) y_length = min(12.0,y_length+1.0);
        else if(!space_done) x_length = min(12.0,x_length+0.05);
        else if(!passes_done) y_passes++;
        delay(500);
      }
      if (buttons & BUTTON_DOWN) {
        if(!length_done) y_length = max(1.0,y_length-1.0) ;
...
int get_data() {
  pinMode(47,INPUT);
  int val = digitalRead(47);//analogRead(C_IN);
  return val;
}
...
int y_pass(float inches, int dir) {
  ...
  int interval_size = 10; //one tenth of a rotation. can probably extend
